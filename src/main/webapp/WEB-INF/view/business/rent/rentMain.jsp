    ...
    <div style="padding:10px 20px 1px 20px;">
        ...
        <div style="display: none;padding: 20px" id="saveOrUpdateDiv">
            <form class="layui-form" lay-filter="dataForm" id="dataForm">
                <div class="layui-form-item">
                    <div class="layui-inline">
                        <label class="layui-form-label">出租单姓名:</label>
                        <div class="layui-input-inline">
                            <input type="text" name="custname" lay-verify="required" placeholder="请输入出租单姓名"
                                   autocomplete="off"
                                   class="layui-input">
                        </div>
                    </div>
                    <div class="layui-inline">
                        <label class="layui-form-label">出租单职位:</label>
                        <div class="layui-input-inline">
                            <input type="text" name="career" lay-verify="required" placeholder="请输入出租单职位"
                                   autocomplete="off"
                                   class="layui-input">
                        </div>
                    </div>
                ...
                <div class="layui-form-item">
                    <div class="layui-inline">
                        <label class="layui-form-label">身份证号:</label>
                        <div class="layui-input-inline">
                            <input type="text" name="identity" id="identity" placeholder="请输入出租单身份证号" autocomplete="off"
                                   class="layui-input">
                        </div>
                    </div>
                    <div class="layui-inline">
                        <label class="layui-form-label">出租单地址:</label>
                        <div class="layui-input-inline">
                            <input type="text" name="address" placeholder="请输入出租单地址" autocomplete="off"
                                   class="layui-input">
                        </div>
                    </div>
                </div>
                <div class="layui-form-item">
                    <div class="layui-inline">
                        <label class="layui-form-label">出租单电话:</label>
                        <div class="layui-input-inline">
                            <input type="text" name="phone" lay-verify="required|phone" placeholder="请输入出租单电话"
                                   autocomplete="off"
                                   class="layui-input">
                        </div>
                    </div>
                    <div class="layui-inline">
                        <label class="layui-form-label">出租单性别:</label>
                        <div class="layui-input-inline">
...
<script type="text/javascript">
    ...
    layui.use(['jquery', 'element', 'form', 'layer', 'table','laydate'], function () {
        ...
        table.on('tool(dataTable)', function (obj) {
            ...
            if (layEvent === 'del') { //删除
                layer.confirm('确定删除出租单[' + data.rentid + ']吗？', function (index) {
                    ...
                    layer.close(index);
                    //向服务端发送删除指令
                    $.post("<%=basePath%>rent/deleteById", {"id": data.rentid}, function (result) {
        ...
        function batchDelete() {
            ...
            list = list.substr(1);
            alert(list);
            layer.confirm('确定删除选中出租单吗？', function (index) {
                layer.close(index);
                //向服务端发送删除指令
                $.post("<%=basePath%>rent/batchDelete", {"ids": list}, function (result) {
                    layer.msg(result.msg);
                    // 刷新数据表格
                    tableIns.reload();      //在不刷新页面的情况刷新表格数据
                })
