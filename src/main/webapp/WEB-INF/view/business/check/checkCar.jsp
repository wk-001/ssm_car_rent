                ...
                <div class="layui-form-item ">
                    <div class="layui-inline layui-col-md6 layui-col-md-offset2">
                        ...
                        <div class="layui-input-block">
                            <input type="text" name="search_rentid" id="search_rentid" class="layui-input">
                        </div>
                    </div>
...
<script type="text/javascript">
    ...
    layui.use(['jquery', 'form', 'layer'], function () {
        ...
        $("#doSearch").click(function () {
            ...
            $.post("<%=basePath%>rent/getById", {rentid: rentid}, function (result) {
                if (result === "") {
                    layer.msg("该出租单号不存在，请检查输入是否正确！");
                    $("#content").hide();
                } else {
                    if (result.rentflag == 1) {
                        layer.msg("该出租单号相关车辆已归还，无需入库！");
                        $("#content").hide();
                    } else {
        ...
        function initCheckFormData(rentid) {
            $.post("${ctx}/check/initCheckFormData", {rentid: rentid}, function (obj) {
                ...
                $("#car_carimg").attr("src", "<%=basePath%>file/downloadShowFile?path=" + car.carimg);
            })
        }
        ...
        form.on("submit(doSubmit)", function () {
            ...
            $.post("<%=basePath%>check/saveCheck", params, function (obj) {
                layer.msg(obj.msg);
                $("#content").hide();
            })
            return false;
